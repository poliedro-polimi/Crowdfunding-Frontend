...
abstract class Model{
  ...
  public function __construct($auto_load=false,$info=array(),$apply_callback=true){
    ...
    elseif(is_array($auto_load) || is_string($auto_load)){
      ...
      foreach($auto_load as $field){
        ...
        elseif(static::isMyAssociation($field) and !array_key_exists(self::normalizeAttributeName($field),$this->loaded_associations)
          and in_array($associations[self::normalizeAttributeName($field)]['assoc'],array(self::JOIN_ONE_TO_ONE,
  ...
  public static function exists($id){
    ...
    $q=DB::query("SELECT COUNT(*) AS N FROM ".static::getTableName()." WHERE ".$key[0].'='.$key[1],true);
    return $q['N']>0;
  }
  ...
  public function __call($name, $arguments){
      ...
      if(!empty($attr_name)){
        if($method=='get'){
          ...
          elseif(static::isMyAssociation($attr_name)){
            ...
            if(!empty($assoc)){
              if(isset($arguments[1])){
                $data=$this->getAssociation($assoc['name'], (array)$arguments[0], (bool)$arguments[1]);
              }
              elseif(isset($arguments[0])){
                if(is_array($arguments[0])){
    ...
    throw new ModelException(self::MODEL_NO_ATTR,1, $name.' non è stato riconosciuto come un getter o setter valido per '
                                          .get_called_class());
  }
  ...
  private function getAssocAndAttr($attr,$assoc,$reload=false){
    ...
    foreach($values as $field=>$value){
      ...
      elseif(static::isMyAttribute($corr_attr)){
        ...
        $out[$corr_attr]=static::normalizeAttribute($corr_attr,'on_get',$this->loaded_attributes[$field], array
        ('this'=>$this));
      }
    }
  ...
  private function getSingleAttribute($name,$reload=false){
    if(static::isMyAttribute($name)){
      ...
      if(!$reload && isset($this->loaded_attributes[$field])){
        return static::normalizeAttribute($name,'on_get',$this->loaded_attributes[$field],array('this'=>$this));
      }
      else{
      ...
      throw new ModelException(self::MODEL_NO_ATTR,1, $name.' non risulta essere un attributo di '.get_called_class());
    }
  }
  ...
  public function setAttribute($name,$value){
    if(static::isMyAttribute($name)){
      ...
      throw new ModelException(self::MODEL_NO_ATTR,1,$name.' non è riconosciuto come attributo di '.get_called_class());
    }
  }
  ...
  private function load($field){
    try{
      if(!empty($field)){
        ...
        $primary=self::normalizeAttribute(static::getIdName(),'write',$this->loaded_attributes[$primary_field]);
        if(is_string($field)){
          $q=DB::query("SELECT ".$field." FROM ".static::getTableName()." WHERE ".$primary[0]."=".$primary[1],true,RESULT_ARRAY);
          if(!empty($q)){
            return $q[0];
          }
          else{
        ...
        elseif(is_array($field)){
          ...
          $q=DB::query("SELECT ".implode(', ', $field)." FROM ".static::getTableName()." AS THIS ".implode(' ', $joins)." WHERE THIS.".$primary[0]."=".$primary[1],true);
          if(!empty($q)){
            return $q;
          }
          else{
    ...
    if(!array_key_exists($name,$this->loaded_associations) || $reload){
      ...
      if(isset($joins[$name])){
        ...
        if(!empty($j['model'])){
          ...
          throw new ModelException("Il modello specificato non esiste",6,$j['model'].' non è un modello riconosciuto');
        }
      }
      else{
        throw new ModelException("L'associazione richiesta non esiste",5, $name." non risulta essere un'associazione di ".get_called_class());
      }
    }
  ...
  public function save(){
    ...
    try{
      $primary=self::normalizeAttribute(static::getIdName(),'write',$this->loaded_attributes[$primary_field]);
      DB::query("UPDATE ".static::getTableName()." SET ".implode(', ', $fields)." WHERE ".$primary[0]."=".$primary[1]);
      return true;
    }
    catch(DBException $e){
  ...
  public static function find($search=array()){
    ...
    if(!empty($search['search_fields'])){
      if(is_array($search['search_fields'])){
              ...
              if(($raw_ass_data['assoc']==self::JOIN_ONE_TO_ONE || $raw_ass_data['assoc']==self::JOIN_MANY_TO_ONE)) {
                if (strpos($f, $a . '.') === 0) {
                  ...
                  if (method_exists($ass_data['model'], 'normalizeAttribute')) {
                    $what[] = $ass_data['model']::normalizeAttribute($ass_f, 'read', null, array('table' => $a,
                                                                                                 'prefix'=> $a . '_'));
                  }
                  else {
                    $what[] = $f.' AS '.$a.'_'.$ass_f;
        ...
        if(!empty($what)){
          $primary_field=self::normalizeAttribute(static::getIdName(),'read', null, array('table' => 'THIS'));
  ...
  protected static function normalizeAttribute($attr,$mode='name',$value=null,$add_opt=array()){
        ...
        switch($mode){
          ...
          case 'read':
            if(is_callable($a_value['read'])){
              $out=call_user_func($a_value['read'],(!empty($add_opt['table'])?$add_opt['table'].".":'')
                                                  .$a_value['name']);
            }
            else{
    ...
    throw new ModelException(self::MODEL_NO_ATTR,1,$attr." non è risconosciuto come attributo di ".get_called_class());
  }
    ...
    foreach($input as $attr){
      if(static::isMyAttribute($attr)){
        ...
        if($reload or !isset($this->loaded_attributes[$field_name])){
          ...
          $out[$attr]=static::normalizeAttribute($attr,'on_get',$this->loaded_attributes[$field_name], array
          ('this'=>$this));
        }
      }
      else{
