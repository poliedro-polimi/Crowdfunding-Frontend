...
abstract class Controller{
    ...
    /**
     * Renders a View file
     * @param string $path the path to the view file. Can be relative to the /views folder or to the root or be absolute
    ...
    protected function renderView($path, $args){
        $p = dirname(__DIR__).'/views/'.$path;
        if(file_exists($p)){
            return page_include($p, $args);
        }
        elseif(file_exists($path)){
            return page_include($path, $args);
        }
        else{
